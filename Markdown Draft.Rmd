...
```{r plot common}
plot(common_fit_m, plotTracks = F)
```
...
```{r map}
ggplot () + geom_point(data=chk11 %>% filter(ID == 5), aes (x=BNGX, y=BNGY))
#plot of tern GPS points in British National Grid coordinate system
```

```{r}
ggplot() +
geom_polygon(data = uk_ukgrid, aes(x = long, y = lat, group = group), colour = "black", fill = NA) +
geom_point(data=chk11 %>% filter(SPECIES == "Arctic"), aes(x=BNGX,y=BNGY, color = ID))+coord_fixed(ratio = 1, xlim = c(min(chk11$BNGX),max(chk11$BNGX)), ylim = c(min(chk11$BNGY), max(chk11$BNGY)) )   #+ylim(5e+05,5.5e+05) + xlim(400000,50000)
```