...
plot2 = ggplot(revise(chk10 %>% filter(SPECIES != "Common/Arctic")) %>% filter(CONTBEH != "REST"), aes(x=SPECIES, y = DISTKM, fill = CONTBEH)) +
  ...
  guides(fill=guide_legend(title="Cont. Behaviour"))
DISTKM_plot = grid.arrange(plot1,plot2, top =  "Continuous behaviour in relation to distance of tern from the colony")
```
```{r}
grid.arrange(DIST2COL_plot, DISTKM_plot, ncol = 2 )
```

```{r select libraries}

# copy dataset and select rows
...
```
```{r plotboth function}
plotcomplete_incomplete = function(df, title = "Title", legendshow = T){
  #
  #
  ...
  df_incom = suppressMessages(plotcomplete(df, str = "Incomplete", complete = 0, legendshow = legendshow))
...
grid1 = plotcomplete_incomplete(df_arctic, title = "Arctic", legendshow = F)
grid2 = plotcomplete_incomplete(df_sand, title = "Sandwich", legendshow = F)
grid3 = plotcomplete_incomplete(df_common, title = "Common", legendshow = F)
