...
```{r plot prepdata step}
#Check differences between dataframes -step
par(mfrow= c(1,2))
hist(arc11.prepdata$step, breaks = seq(0,0.019, l = 30))
hist(arc11.prepdata_revised$step, breaks = seq(0,0.019, l = 30))
```
```{r plot prep data angle}
#check differences between dataframes - angle
par(mfrow= c(1,2))
hist(arc11.prepdata$angle, breaks = seq(-pi,pi,length =50))
hist(arc11.prepdata_revised$angle, breaks = seq(-pi,pi,length =50))
```
...
\
```{r AIC}
AIC(arc11_basic_m, arc11_revised_m, m1, disthmm)
```
\
...
#     obj: actual object of dataframe we want to fit
...
\
```{r parallel arcic}
arctic_parallel_m = fit_experiment("arc11_revised_m", arc11_revised_m)
```
...
#### Confusion Matrices
\
Transit Search (TS) along with direct flight (DF) are described by higher flight speed and low turns, something that is captured by State 2. On the other hand, Active search (AS) is quite the opposite and will be recoded as state 1. The confusion matrices are given below for all models.\
```{r confusion matrix basic}
states = viterbi(arc11_basic_m) %>% as.factor()
...
```
\
A custom function will be implemented to make heatmaps from each confusion matrix.\
\
...
ggplot(as.data.frame(matrixobject$table), aes(x=Reference,y=Prediction, fill = Freq/sum(Freq))) + geom_tile() + theme_bw() +
   ...
   geom_text(aes(label= paste(100*round(Freq/sum(Freq),4),"%"))) + 
    ...
    annotate("text", x=0.7,y =0.05, label = paste("Overall Acurracy:", 100*as.numeric(round(matrixobject$overall[1],3)),"%" ))+
...
statesadd = viterbi(disthmm) %>% as.factor

conf_arc_add = confusionMatrix(statesadd, recodedtest)
...
\
```{r sand plot}
plot(sand_fit_m,  plotTracks = F, title = "Sandwich 2011")
```
#### Confusion Matrix
