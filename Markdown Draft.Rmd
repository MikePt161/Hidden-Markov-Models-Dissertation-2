...
```{r}
arc11.prepdata_revised =  arc11.prepdata %>% 
...
arc_revised_m = fitHMM(data = arc11.prepdata_revised, nbStates = nbStates, stepPar0 = stepPar0, anglePar0 = anglePar0, formula = formula)
```

```{r plot revised }
plot(arc_revised_m,plotCI = T, plotTracks = FALSE, breaks = seq(0,0.019,l=60))
```
\
It has been concluded that adding more variables worsens model performance.\
```{r additional models}
disthmm = fitHMM(data = test, nbStates = nbStates, stepPar0 = stepPar0, anglePar0 = anglePar0, formula = ~ DIST2COL + BRG2COL + DISTKM + TSECS)
```

\ 
The AIC values will ensure that we have chosen the correct model (~1) for the analysis. As it will be shown, the acurracies between the test and 
```{r AIC}
AIC(arc_basic_m, arc_revised_m, m1, disthmm)
```
\
The following procedure helps with running multiple iterations with randomised initial values, to ensure model has converged. The "parallel" package will assist with lower computation time. \
\
```{r parallel}
...
m <- fitHMM(data = arc11.prepdata_revised, nbStates = 2, stepPar0 = par0$step,
...
statestest = viterbi(arc_revised_m) %>% as.factor
